...
class MJCFRootComponent(ABC):
    ...
    def attach(
        ...
        attachment_site = self.mjcf_body.add(
                'site', name=f"{self.base_name}_attachment_{other.base_name}", pos=position, euler=euler
                )
        other._parent = self
    ...
    @property
    def coordinate_frame_in_root_frame(
            self
            ) -> (np.ndarray, np.ndarray):
        return np.zeros(3), Rotation.from_euler('xyz', [0, 0, 0]).as_matrix()

    def coordinates_of_point_in_root_frame(
            self,
            point: np.ndarray
            ) -> np.ndarray:
        return point
...
class MJCFSubComponent(ABC):
    ...
    def attach(
            ...
            other: MJCFRootComponent,
            position: Optional[np.ndarray] = None,
            euler: Optional[np.ndarray] = None, ) -> None:
        attachment_site = self.mjcf_body.add(
                'site', name=f"{self.base_name}_attachment_{other.base_name}", pos=position, euler=euler
                )
        other._parent = self
    ...
    def coordinate_frame_in_root_frame(
        ...
        if self._coordinate_frame_in_root_frame is None:
            ...
            self._coordinate_frame_in_root_frame = parent_origin + parent_rot.dot(pos), parent_rot.dot(my_rot)
